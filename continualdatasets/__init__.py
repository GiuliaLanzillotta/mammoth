...
def get_all_models():
    return [model.split('.')[0] for model in os.listdir('continualdatasets')
            if not model.find('__') > -1 and 'py' in model]


NAMES = {}
for model in get_all_models():
    mod = importlib.import_module('continualdatasets.' + model)
...
from continualdatasets.cifar5m import *
