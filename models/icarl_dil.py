...
def fill_buffer(self, mem_buffer: Buffer, dataset, t_idx: int) -> None:
    ...
    self.net.eval()
    samples_per_task = mem_buffer.buffer_size // len(self.tasks_so_far)
    ...
    mem_buffer.add_data(
            examples=_y_x[:samples_per_task],
            labels=_y_y[:samples_per_task],
            logits=_y_l[:samples_per_task]
    )
    ...
    # 2.1 Extract all features from the new samples
    a_x, a_y, a_f, a_l = [], [], [], []
    ...
    for _y in a_y.unique():
        ...
        i = 0
        while i < samples_per_task and i < feats.shape[0]:
...
class ICarlDil(ContinualModel):
    NAME = 'icarl_dil'
    COMPATIBILITY = ['domain-il']

    def __init__(self, backbone, loss, args, transform):
        super(ICarlDil, self).__init__(backbone, loss, args, transform)
        self.dataset = get_dataset(args)

        # Instantiate buffers
        self.buffer = Buffer(self.args.buffer_size, self.device)
        self.eye = torch.eye(self.dataset.N_CLASSES_PER_TASK).to(self.device)
